...
import taichi as ti
from dsfilter.R3.utils import sanitize_reflected_index
...
def convolve_with_kernel_x_dir(
    ...
    Args:
      ...
      Mutated:
        ...
        `u` with `k`.
    """
    for x, y, z in u_convolved:
        s = 0.
        for i in range(2*radius+1):
            index = sanitize_reflected_index(ti.Vector([x - radius + i, y, z], dt=ti.i32), u)
...
def convolve_with_kernel_y_dir(
    ...
    for x, y, z in u_convolved:
        s = 0.
        for i in range(2*radius+1):
            index = sanitize_reflected_index(ti.Vector([x, y - radius + i, z], dt=ti.i32), u)
            s+= u[index] * k[2*radius-i]
        u_convolved[x, y, z] = s

@ti.func
def convolve_with_kernel_z_dir(
    u: ti.template(),
    k: ti.template(),
    radius: ti.i32,
    u_convolved: ti.template()
    ...
    for x, y, z in u_convolved:
        s = 0.
        for i in range(2*radius+1):
            index = sanitize_reflected_index(ti.Vector([x, y, z - radius + i], dt=ti.i32), u)
            s+= u[index] * k[2*radius-i]
        u_convolved[x, y, z] = s
